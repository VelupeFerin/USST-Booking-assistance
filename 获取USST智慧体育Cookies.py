...
async def main():
    input(
        "即将打开Chrome浏览器（由本程序打开，若未打开则说明没有安装Chrome或打开过程中出现问题），并跳转到USST智慧体育个人中心。请登录然后回到此处。点击Enter打开浏览器")
    ...
    browser.stop()
    input("您的cookies已保存到本程序所在文件夹\n接下来要检查cookies是否有效。点击Enter继续")
    browser = await nd.start()
    await browser.get('https://usst.ydmap.cn/user/my')
    try:
        await browser.cookies.load(cookies_file_name + '.cookies')
        input("如果在USST智慧体育个人中心发现您已登录，那么刚才导出的cookies是有效的。点击Enter关闭")
    except Exception:
        input("打开cookies时出现异常，请重新执行本程序")
    browser.stop()
